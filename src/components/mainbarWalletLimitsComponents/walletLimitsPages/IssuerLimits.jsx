...
import './IssuerLimits.css'
import { useOutletContext } from 'react-router-dom'
import axios from 'axios'
const IssuerLimits = () => {
    const {issuerLimits,setIssuerLimits} = useOutletContext()
    ...
    const handleSave = async () => {
        const updatedItem = { ...issuerLimits.find(item => item.id === editingField.id) };
        updatedItem[editingField.field] = inputValue;
      
        try {
          await axios.put(`http://localhost:3002/IssuerLimits/${editingField.id}`, updatedItem);
      
          // Update UI locally without reload
          const updatedList = issuerLimits.map(item =>
            item.id === updatedItem.id ? updatedItem : item
          );
          setIssuerLimits(updatedList);
  ...
  return (
    <div className='issuerlimits'>
        {issuerLimits.map((item) =>{
            return(
                <>
                    <div className="cards">
