...
import './RegulatoryAuthorityLimits.css'
import { useOutletContext } from 'react-router-dom'
import axios from 'axios'
const RegulatoryAuthorityLimits = () => {
    const {regLimits, setRegLimits} = useOutletContext()
    ...
    const handleSave = async () => {
        const updatedItem = { ...regLimits.find(item => item.id === editingField.id) };
        updatedItem[editingField.field] = inputValue;
      
        try {
          await axios.put(`http://localhost:3002/RegulatoryAuthorityLimits/${editingField.id}`, updatedItem);
      
          // Update UI locally without reload
          const updatedList = regLimits.map(item =>
            item.id === updatedItem.id ? updatedItem : item
          );
          setRegLimits(updatedList);
  ...
  return (
    <div className='RegulatoryAuthorityLimits'>
        {regLimits.map((item) =>{
            return(
                <>
                    <div className="cards">
...
export default RegulatoryAuthorityLimits
